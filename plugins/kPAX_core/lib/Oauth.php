...
class KpaxOAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod_RSA_SHA1 {
    ...
    public function fetch_private_cert(&$request) {
        // SECURITY: an RSA private signing key was previously hardcoded here and
        // committed to the repository. It has been removed and MUST be rotated.
        // Load the private key from a file outside the web root or from
        // configuration/environment instead of embedding it in source.
        $keyPath = getenv('KPAX_OAUTH_PRIVATE_KEY_PATH');
        $cert = ($keyPath && is_readable($keyPath)) ? file_get_contents($keyPath) : '';
        return $cert;
    }
