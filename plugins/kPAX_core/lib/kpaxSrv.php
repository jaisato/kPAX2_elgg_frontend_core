...
class kpaxSrv {
    ...
    private $key;
    private $username;
    // SECURITY: API key must be loaded from environment, not hardcoded in source code
    private $apiKey;
  ...
  public function __construct($username) { //TODO: ha de ser kPAXadmin o admin per defecte???
    ...
    // Load API key from environment instead of hardcoded value
    $this->apiKey = getenv('KPAX_API_KEY');
    if (!$this->apiKey) {
        error_log('WARNING: KPAX_API_KEY environment variable is not set.');
    }
