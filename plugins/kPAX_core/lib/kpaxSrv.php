...
class kpaxSrv {
    ...
    protected $url = "http://localhost:8081"; // TODO: Use HTTPS in production
    private $key;
    private $username;
    // SECURITY: API key should be loaded from Elgg plugin settings, not hardcoded
    private $apiKey;  // Set via elgg_get_plugin_setting('api_key', 'kPAX_core')
  ...
  public function __construct($username) { //TODO: ha de ser kPAXadmin o admin per defecte???
    ...
    // Load API key from plugin settings instead of hardcoded value
    $this->apiKey = elgg_get_plugin_setting('api_key', 'kPAX_core');
    if (empty($this->apiKey)) {
        error_log('kPAX: WARNING - API key not configured in plugin settings');
    }
