...
class kpaxSrv {
    ...
    protected $url = "https://localhost:8081";
    private $key;
    private $username;
    // SECURITY FIX: API key must not be hardcoded. Load from environment or config file outside web root.
    private $apiKey = null; // Set via constructor or elgg plugin settings
  ...
  public function __construct($username) { //TODO: ha de ser kPAXadmin o admin per defecte???
    ...
    // SECURITY FIX: Load API key from elgg plugin settings instead of hardcoding
    $this->apiKey = elgg_get_plugin_setting('api_key', 'kPAX_core');
    if (empty($this->apiKey)) {
        error_log("WARNING: kPAX API key not configured in plugin settings");
    }
